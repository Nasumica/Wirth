...
function LookAndSayPoly(z: complex): complex; overload;
...
function cagm(x, y: numeric): numeric; // cubic arithmetic-geometric mean
const
  maxiter = 64;
var
  i: integer;
...
begin
  if x = y then result := x else begin
    ...
    while (result <> x) and (result <> y) and (i < maxiter) do begin
      y := result;  z := x;  i := i + 1;
      result := (x + 2*y)/3; x := cbrt((cub(y) + x * y * (x + y))/3);
      if x = z then result := x;
    end;
  end;
end;
...
function LookAndSayPoly(z: complex): complex;
const
  ƛ = 1.3035772690342963912570991121525518907307025046594; // Conway constant
begin
  if z = ƛ then result := 0 else
    result := poly(z, [
      -6, 3, -6, 12, -4, 7, -7, 1, 0, 5, -2, -4, -12, 2, 7, 12, -7, -10,
      -4, 3, 9, -7, 0, -8, 14, -3, 9, 2, -3, -10, -2, -6, 1, 10, -3, 1, 7,
      -7, 7, -12, -5, 8, 6, 10, -8, -8, -7, -3, 9, 1, 6, 6, -2, -3, -10,
      -2, 3, 5, 2, -1, -1, -1, -1, -1, 1, 2, 2, -1, -2, -1, 0, 1
    ]);
end;
